...
class AABB2D {
 public:
  ...
  auto inline contains_point(const float x, const float y) const -> bool {
    return x >= min_coords.first && x <= max_coords.first &&
           y >= min_coords.second && y <= max_coords.second;
  }
  auto inline contains_point(const std::pair<float, float>& coords) const
      -> bool {
  ...
  auto inline interesects(const AABB2D& other) const -> bool {
